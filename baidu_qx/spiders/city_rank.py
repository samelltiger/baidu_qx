...
from baidu_qx.items import CityRankItem
...
class mingyan(scrapy.Spider):  # 需要继承scrapy.Spider类
    ...
    name = "city_rank"  # 定义蜘蛛名

    def start_requests(self):  # 由此方法通过下面链接爬取页面
        url = "http://huiyan.baidu.com/migration/cityrank.jsonp?dt=city&id={0}&type=move_in&date={1}"
    ...
    def parse(self, response):
        ...
        for data in json_dict:
            item["city_name"] = eara
            item["inOrOUt_city"] = data['city_name']
            item["inOrout_city_province_name"] = data['province_name']
